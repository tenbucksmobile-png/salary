...
**New Gross Salary — post-increase gross, sourced from Salary Review, feeds the Bonus calc.** `fetchScenarioLineMap()` resolves each hotel's winning `increase_scenarios` row independently (draft wins if one exists, else that hotel's own most recent approved/applied/committed scenario) — unlike `SalarySummaryTable`'s single global committed-scenario fallback, this is per-hotel since the export spans all seven Leave/Bonus hotels at once and a single global fallback would only ever reflect one hotel's latest commit. `newGrossMap` reconstructs true gross the same way `SalarySummaryTable`'s `computeEmployeeFigures()` does — `scenario_lines.new_basic + allowances.structure` — falling back to the employee's current Gross Salary when they have no scenario line (excluded from the increase, or no scenario at all for that hotel). This is a single Employee-block column, not duplicated into the Leave or Bonus segment blocks. **Bonus Required (Dec)** (and therefore Provision Balance) for non-incentive employees now multiplies the Payout Factor against this New Gross Salary instead of current Gross Salary; incentive-scheme employees are unaffected (they still use their `salary_records.incentive` rate, which has no gross-salary dependency). The Bonus segment's own displayed `Gross Salary` column is untouched — still the employee's current gross, for reference.
