...
**Export** (`buildItw8Csv()`) produces the exact ITW8 shape — 25 columns, **semicolon**-delimited (`padRow()` pads every row to 25 fields with a trailing-semicolon tail). Verified directly against BURS's own published `itw8_paye_template.csv` (obtained from the BURS portal, 2026-08) — header labels, the 25 column names, and the 4-row structure (label row, values row, column-header row, data rows) all match it exactly. One `TaxYear;TaxMonth;EmployerTin;EmployerName` row, then the fixed column-header row, then one row per taxpayer.

**Delimiter history — trust the official template, not an inferred sample**: this was briefly changed to comma-delimited based on a "June 2026" ILG file the user described as an accepted submission, on the assumption that file was ground truth. That assumption was wrong (or at least unconfirmed) — after the comma version *also* produced the same BURS-side `ArrayIndexOutOfBoundsException` on upload, the user retrieved BURS's actual template from the portal, which is unambiguously semicolon-delimited, matching the original implementation. **A filled example file of unconfirmed provenance is not a reliable spec — get the actual blank template from the source system before trusting its delimiter/structure over your own reasonable default.** If BURS ever rejects an export again, do not immediately re-guess the delimiter — the standing hypothesis is that the user's Excel round-trip (opening the downloaded CSV to fix ID/date formatting, then re-saving) re-serializes the file using Excel's own regional list-separator setting, silently overriding whatever delimiter the app generated, regardless of which one it picks. The fix for that is to upload the app's raw generated file directly without opening/editing it in Excel (use a plain text editor if a manual check is needed) — not to keep changing `padRow()`'s delimiter.
