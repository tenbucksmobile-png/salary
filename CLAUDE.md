...
**Export** (`buildItw8Csv()`) produces the exact ITW8 shape from `itw8_paye_template.csv` — 25 columns, `;`-delimited, verified byte-for-byte against the template's header/column rows (`padRow()` pads every row to 25 fields, matching the template's trailing-semicolon padding exactly). One `TaxYear;TaxMonth;EmployerTin;EmployerName` row, then the fixed column-header row, then one row per taxpayer. Field mapping confirmed/corrected against a real submitted ILG ITW8 export (June 2026):
