...
**Years Service / Months Service (Employee block) and Yrs Service (Severance block) are now live formulas anchored to 31 July of the selected Year** (`=ROUND(DATEDIF(StartDateCell,DATE(year,7,31),"d")/365.25,1)` for years, `=DATEDIF(StartDateCell,DATE(year,7,31),"m")` for months) rather than static values computed off "today" — a fixed, reproducible snapshot date matching Gratuity's own existing 31-July anchor (`periodOfAccrualMonths`). **Bonus's Mths Service (Dec) is a separate live formula anchored to 31 December of the same Year** (`=DATEDIF(StartDateCell,DATE(year,12,31),"m")`), matching the 13th-cheque payout factor's own existing Dec-31 basis (`monthsOfServiceAtDec`). All three reference the same Start Date cell in the Employee block, so editing an employee's Start Date and recalculating updates every one of these columns consistently. This only changed how the export **displays** these figures — the underlying JS `yearsOfService()`/`monthsOfService()` helpers (still "today"-anchored) are untouched everywhere else in this file, including the Severance business logic (Days/Month threshold, Months Accrued) that still drives the actual Severance Balance figure — so a row's displayed Yrs Service (31-July-anchored) and its Days/Month value (still "today"-anchored) can, in principle, disagree by a few days around the 5-year boundary. Revisit if that's ever a problem in practice.
...
**Cells carry live Excel formulas, not just static numbers**, wherever the derivation is a safe, unconditional function of other cells already on the same row: per-hotel sheets' Leave Daily Rate and Provision Value (`=DailyRate×CappedBalance`), Severance Daily Rate and Provision Balance (`=MonthlyRate×MonthsAccrued`), Years/Months Service and Mths Service (Dec) (see the anchor-date paragraphs above), Bonus Payout Factor (`=MIN(MonthsCol,12)/12`), and the totals row's per-segment column sums (safe even when some rows are blank text, since Excel's `SUM` ignores non-numeric cells). The Overview sheet's Adjustment columns are `=FLOOR.MATH(Required−OnBooks,100)`, matching the standalone pages' own `Math.floor((cost-book)/100)*100` JS exactly (`FLOOR.MATH` with default mode rounds negative numbers toward −∞, same as `Math.floor`). Bonus's own **Bonus Required (Dec)** and **Provision Balance** columns, and Gratuity's own **Provision Balance** column, stay static values, not formulas — their derivation branches on `incentive_applicable`, the <6-month forfeiture rule, or the FRE001/FRE002 period-of-accrual override, none of which is exposed as its own column in the sheet, so formularizing them risks silently producing a wrong value if a user edits an upstream cell in a way that violates the hidden business rule.
