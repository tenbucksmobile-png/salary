...
**ANO (vacant position) rows are always zeroed** — `employee.grade_label === 'ANO'` forces every column to `—`/0 regardless of what's on the placeholder salary record, since a vacant position isn't a real employee. Display-only — doesn't touch `salary_records`.
...
**FRE001/FRE002 override (two-tranche contract, revised 2026-08-06)** — their actual contract wording: *"A gratuity of 10% of basic salary is payable in two tranches, the first tranche will be payable in September 2026 for the first completed two years, with the second payable on successful completion of your contract."* This is **10% of BASIC salary per completed 24-month tranche**, not a flat monthly/annual accrual — a materially different shape from every other gratuity-applicable employee (who use plain Gross × Rate%, no tranches, no Basic/Gross distinction). `GRATUITY_ACCRUAL_START` (page-local, hardcoded to these two employee codes only) sets an LOA start date of `2024-10-01`; `GRATUITY_TRANCHE_MONTHS = 24`. Their Gratuity Balance as of any cutoff = `Basic Salary × Rate% × (min(periodOfAccrualMonths(startDate, year), 24) / 24)` — the pro-rata portion of the **first** tranche's 24-month period elapsed so far (e.g. 1 Oct 2024 → 31 Jul 2026 = 21 months elapsed → 21/24 = 87.5% of tranche 1 accrued). The second tranche (contract completion) has no fixed end date, so it isn't accrued for at all here — only tranche 1's pro-rata is computed. Two earlier, incorrect versions preceded this: a Gross×Rate%×(days÷365.25) fraction-of-a-year multiplier, then a Gross×Rate%×whole-months-multiplied-directly version (which wildly overstated the balance, e.g. 10%×21=210% of gross) — both were replaced once the actual contract text was provided. Every other gratuity-applicable employee keeps the plain Gross × Rate% formula untouched.
...
**Not yet reflected in the Provisions Overview combined export** (`src/lib/provisions-export.ts`) — that export's own `SEVERANCE_HOTEL_CODES = ['ILG']` and severance-only (no gratuity) scope were not touched by this widening; it still only pulls ILG severance data. Revisit if the combined export needs to match this page's scope.
