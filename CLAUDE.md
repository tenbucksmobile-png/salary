...
**`matchCfeEmployee(name)` matches on a single shared name token (surname OR first name, ≥3 letters), not the full name as an exact set — deliberately looser than `nameKey()`.** CFE Management's own deductions report frequently uses initials for first names (`"MR B.A. BAAKILE"`), while the CSL/NL statement it's cross-checked against uses the full name (`"BABOLOKI BAAKILE"`); an exact `nameKey()` match would never link these even though the surname alone makes it unambiguous. `cfeTokenIndex: Map<token, Employee>` is built once from all CFE employees' surname + first-name tokens; a statement line matches if any of its tokens (≥3 letters, titles stripped) hits that index. Confirmed against real July 2026 data: this closed most of the gap for CB Stores/Topline/Furnmart (e.g. CB Stores went from an exact-match total of R2,541 to R4,838 against CFEM's own R4,398 report — much closer, previously most of the roster was silently unmatched).

**Known limitation — this looser matching trades some false positives for catching the initials case**: confirmed against the same real data, two CSL employees who merely *share* a first name or surname with a CFE employee (but aren't the same person — e.g. a different "Dorcus" than CFE's Dorcus Shamukani, and a different "Nkwazi" than CFE's Thomas Nkwazi) get pulled into the embedded total as false positives. The `onlyInCfem`/`onlyInEmbedded` callout is what surfaces these for a human to sanity-check — treat unexpected names in that list as a signal to verify, not just genuine gaps. The `onlyInCfem`/`onlyInEmbedded` diff also now resolves each line to a CFE `Employee` via `matchCfeEmployee()` and compares by employee `id`, not by raw name string — this was necessary once matching became token-based, otherwise a correctly-matched Baakile would still show up as "unmatched on both sides" in the diff listing despite counting correctly in the totals above it.

**Known gap — Bodulo can never be name-matched for CFE at all**: `parseBodulo()`'s `name` field is always just the employee code repeated (confirmed: `{ name: "EMP00316", empCode: "EMP00316" }`), never a real name — this predates the CFE cross-reference work and isn't something `matchCfeEmployee()` can fix; it needs the Bodulo parser itself extended to actually read a name column, which requires checking that file's real layout first (not done — don't assume a fix here without inspecting an actual Bodulo export).

**Known data-quality gap, not a matching bug**: CFE employee `Shamukani, Dorcus` in the `employees` table is spelled `Shamukuni` in both the CFEM report and CSL's own statement — consistent with each other, inconsistent with the DB. Until the DB record is corrected (Employees page), she'll never resolve via `matchCfeEmployee()` from either source, since the token index is built from the DB's spelling.
