...
**Export** (`buildItw8Csv()`) produces the exact ITW8 shape — 25 columns, **comma**-delimited (`padRow()` pads every row to 25 fields with a trailing-comma tail, matching a real BURS-submitted ILG ITW8 file exactly). One `TaxYear,TaxMonth,EmployerTin,EmployerName` row, then the fixed column-header row, then one row per taxpayer. **This was previously semicolon-delimited** (`;`) based on an unverified assumption about `itw8_paye_template.csv` — that produced a file BURS's own upload rejected; fixed by comparing byte-for-byte against the real June 2026 sample, which is unambiguously comma-delimited. If BURS ever rejects an export again, diff the downloaded file against a known-good prior submission before assuming the field *values* are wrong — a structural/delimiter mismatch is easy to miss by eye. Field mapping confirmed/corrected against a real submitted ILG ITW8 export (June 2026):
