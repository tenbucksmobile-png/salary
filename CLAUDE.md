...
**FRE001/FRE002 override (period-of-accrual, revised 2026-08-06)** — `GRATUITY_ACCRUAL_START` (page-local, hardcoded to these two employee codes only) sets an LOA start date of `2024-10-01`. Their Gratuity Balance is `Gross Salary × Rate% × periodOfAccrualMonths(startDate, year)`, where `periodOfAccrualMonths()` is the **whole calendar months** from the start date to 31 July of the selected Year (e.g. 1 Oct 2024 → 31 Jul 2026 ≈ 21 months), multiplied **directly** into the formula — not divided into a fraction-of-a-year rate. This was corrected from an initial days÷365.25 fraction-of-a-year version, which undercounted relative to the intended whole-months multiplier. Every other gratuity-applicable employee keeps the plain Gross × Rate% formula untouched.
