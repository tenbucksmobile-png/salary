...
- **Columns parsed**: Surname, First Name, Employee Code, Omang / National ID (`id_number`), Gross Salary, Structure, Basic Salary, Medical Company, Department, Job Title, Grade, Start Date
- **Combined-name columns**: a single "Employee Name"/"Full Name" column (`"MR Modimoosi Lala Baakile"`), or a bare "Name" column when there's no separate Surname column at all (`"NAME, Basic Salary"`), is split token-wise — salutation stripped, last token = surname, everything before it = first name — the same convention `isLeaveBalanceFile` already used. A bare "Name" header is only treated as first-name-only when a real Surname column exists to pair it with; otherwise it's rerouted through this same splitter.
- **Structure / Basic Salary columns**: when a file carries dedicated `Structure` and `Basic Salary` columns alongside Gross (`Basic Salary = Gross − Structure`, same relationship the employee detail page enforces), `basic_salary` is written from the Basic Salary column and `allowances.structure` from the Structure column — **not** the whole Gross figure crammed into `basic_salary` with `allowances: {}`, which was the behaviour before this was fixed and inflates every basic-driven calculation (provident fund, leave/bonus accrual, …) by the structure amount. `TSVEmployee.basicSalaryFile` is `null` (not 0) when no dedicated Basic Salary column exists at all, so the import page falls back to `basicSalaryFile ?? grossSalary` — old files without a Structure split behave exactly as before. `net_salary` on the minimal salary record below is `total_earnings` (gross), not `basic_salary`, since basic no longer equals gross once structure is split out. Provident fund itself needs no special-casing here — `calculateBurden()` already computes EE from basic (and ER from basic too, except APA employees whose job title matches `isDirector()`, where ER uses gross via `PF_ER_APA_DIRECTOR` — see the APA Director override in Payroll Burden Calculations above).
...
- **Update path** (existing employees): writes surname, first_name, job_title, department_code, employee_code (if in file), id_number (if in file), grade_label, employment_date, **and `status: 'active'`** (2026-08-06). HR list is treated as authoritative for names — surname/first_name are updated. **Reactivates a previously `terminated`/`on_leave` employee** if they're matched again in a fresh HR List upload — a full-roster upload is a positive statement "this person is currently on the roster," so a match should never leave a stale non-active status in place. Confirmed live: a vacant "ANO" position at ILG had `status: 'terminated'` from some earlier action; every subsequent HR List re-import matched it and refreshed `last_seen_at` but (before this fix) never touched `status`, so it silently stayed excluded from every active-only query app-wide — including the Dashboard headcount, which is why ILG's Dashboard showed 3 ANO instead of the real 4. The Dashboard itself queries Supabase live with no caching layer, so this was never a "refresh" problem — always check `employees.status` and `grade_label` for stray values before assuming a display bug when a headcount looks short.
  - **Name-matched HR List re-imports can create a duplicate employee instead of updating one, when the file's spelling of a name differs from what's already in the DB** — confirmed live on CSL's Fixed Term roster (no employee codes exist there, migration 014): 8 people (e.g. "Gabakgatlhe" vs "Gabokgatlhe", "Nsengwa Lawarence" vs "Lawrence") got a second employee row each because exact name matching can't bridge a spelling difference, silently doubling that month's headcount. There's no fuzzy-match fix for this — the practical remedy is a one-off DB merge (keep the original row for its salary history, adopt the new upload's spelling/code, move that period's salary record across, delete the duplicate row) whenever a hotel's headcount looks inflated right after an HR List import with no corresponding new hires.
...
**Dashboard** — `SalarySummaryTable` reads all `draft` scenario lines first (shows pending increases before commit). **Once a hotel's scenario is committed, its Increase+Adj column goes back to zero** — a committed increase is already folded into `salary_records` as the new current figure, so there's nothing pending left to highlight; only draft (not-yet-committed) scenarios drive a nonzero delta. Three-tier drill-down, each level with its own "+"/"−" toggle: **Hotel row** (filtered total) → **Grade rows** (one per grade present among the filtered employees at that hotel, sorted by `GRADE_ORDER`, each a subtotal) → **Employee rows** (individuals making up that grade's subtotal). All three levels share the same `computeEmployeeFigures()` logic, so hotel, grade, and employee figures always reconcile. This replaced the old per-hotel `HotelCard` grade-breakdown cards that used to render below the summary table.

**Incident: committed-scenario fallback showed one hotel's stale increase while every other hotel showed none** — an earlier version fell back to "the single most recently committed scenario across *all* hotels" whenever no draft existed anywhere (`ORDER BY committed_at DESC LIMIT 1`). This silently breaks the moment more than one hotel is committed in the same batch — this app's own multi-hotel Commit button always stamps every hotel with the identical `committed_at`, so the tie-break picks one hotel arbitrarily (confirmed live 2026-08-26: all 7 hotels committed at the exact same timestamp, yet only ILG's Increase+Adj rendered — every other hotel showed "—" despite being equally "just committed"). Removed the fallback entirely per explicit direction: a committed hotel with no draft now always falls through to `computeEmployeeFigures`'s no-scenario-line branch (Current Gross === New Gross, zero delta), consistent across every hotel. `fetchScenarioLineMap()` in `src/lib/scenario-lines.ts` (used by Bonus Provision / Provisions Overview) already resolved this correctly on a per-hotel basis and even carried a comment flagging the Dashboard's version as wrong — if a per-hotel "just committed" delta is ever wanted back on the Dashboard, use that helper's approach, not a single global fallback.
...
| Pension Schedule | `pension` | `.xls/.xlsx` | The fund administrator's own monthly contribution schedule; **uploaded per hotel, including CFEM** — unlike the other 5 vendors, pension is never mixed into CSL's/NL's shared statements, so it isn't part of the CFEM Deductions Summary or the CFE Cross-Reference below |
| Pension Deductions (Payroll) | `pension_deductions` | `.csv/.txt` | **CFEM only** — CFEM's own payroll system's pension deductions report ("LIST OF: Pension Fund"), checked against the Pension Schedule above on the Deductions Check tab. A genuinely separate document from a different source (CFEM's own payroll vs. the fund administrator) — the two used to share the `pension` slot (routed by file extension), silently clobbering each other |
...
Re-uploading any slot replaces it (upsert on `period_id, upload_type`). `visibleUploadConfigs` filters which slots render per hotel: CFEM sees `cfem_deductions` + `pension` + `pension_deductions` (`CFEM_UPLOAD_TYPES`) — no other slot, since Payroll Spreadsheet is a salary document CFEM must never upload here. Every other hotel sees everything except `cfem_deductions`/`pension_deductions` (`NON_CFEM_UPLOAD_TYPES`).
...
- The monthly sheet's header (detected by keyword, not a fixed row) has `EMPLOYEE NO`, `FIRST NAMES`, `SURNAME`, `MEMBER CONTRIBUTION AMOUNT` (the **employee/EE** side — what payroll's own `pensionEe` column represents), `EMPLOYER CONTRIBUTION AMOUNT` (**ER**), and — on some schedules, e.g. the CFEM management template — `MEMBER AVC CONTRIBUTION` (an additional voluntary contribution still deducted from the employee). **The combined figure (`bankTotal`) is always computed directly as `EE + ER + AVC`, never read from the sheet's own printed "Total Contributions" column** — confirmed live on the CFEM template that its own total column silently excludes AVC (EE 1,186.50 + ER 2,135.70 + AVC 496.67 prints as "3,322.20", dropping the 496.67). **AVC is folded into the EE-side total too** (`lines[].amount`/`total` = Member Contribution + Member AVC) — confirmed on SHA001 (Dorcus Shamukuni): the 440.15 gap between the Schedule and the Payroll Deductions Report narrowed to 56.52 once her 496.67 AVC was added to the EE side.
- Skips a trailing "TOTAL"/"GRAND TOTAL" summary row on the schedule (checked against the code/first-name/surname cells) — unlike every other vendor parser, this loop originally had no such guard, so a schedule's own totals row got summed in as if it were another employee, silently doubling `total`/`bankTotal` on top of the already-complete per-employee sum.
- **`lines[].amount` and `ParsedStatement.total` are EE-only** (Member Contribution + AVC, excluding ER) — the Deductions Check summary/Employee Detail tables compare this against payroll's `pensionEe` exactly like Furnmart/Bodulo (code-based matching only, no `matchByName`), so the two sides are always like-for-like (payroll never reports an employer contribution at all).
- The combined EE+ER(+AVC) figure is carried separately on `ParsedStatement.bankTotal` — **only the Consolidation tab's Pension "System" figure reads this** (via `getPensionBank()` in `loadSystemTotals`, falling back to `total` for any statement with no EE/ER split), since that combined amount is what's actually paid to the fund administrator each month, not the EE-only deduction. Every other vendor's Consolidation figure and the Deductions Check comparison both read the same `total` — pension is the only line item where those two numbers deliberately differ.
- Each employee line also carries `bankAmount` (the combined EE+ER+AVC figure for that one employee) on the shared `ReconLine` type — needed by the CFE Cross-Reference's per-employee views, which otherwise only have the EE-only `amount` to work with.

**CFEM's Pension Deductions Report** (`pension_deductions`, parsed by `parseCfemPensionCsv`) reuses `parseCfemDeductions`'s "LIST OF: <Vendor> METHOD NO: ALL" section scanner for its own single "Pension Fund" section — same EMP.CODE/NAME/CO.CONTRIB(ER)/EMP.AMOUNT(EE)/TOTAL shape as every other CFEM deductions section. `lines[].amount` is EE-only (`empAmount`), matching the Schedule's own EE-only figure so the **Pension — Schedule vs Payroll Deductions** card on CFEM's Deductions Check tab (a Statement/Payroll/Difference summary plus a per-employee-code detail table, mirroring the pattern CSL/NL get for every other vendor) compares like-for-like.

**CFE Management employees embedded in NL's own Pension Schedule**: confirmed live — MOJ001 (Luengo Mojewa), PHO001 (Boikhutso Phofu, spelled "Tiny Phofu" on NL's file — a nickname override, see below), TSH001 (Presly Tshekedi), TSH002 (Hildah Tshekedi) sit on **NL's** pension schedule, not CFEM's own. `isEmbeddedCfePensionLine()` (reconciliation/page.tsx) identifies these via `matchCfeEmployee()` (name-based — see "CFE Management" below), plus a small hardcoded nickname-override map for cases the generic surname+initial rule can't catch (PHO001's "Tiny" vs her DB first name "Boikhutso"). This powers the CFE Cross-Reference's per-employee Pension visibility (embedded lines pulled into the Schedule side of the comparison, tagged with a "Found In" column) — **but this money is deliberately NOT moved between hotels on the Consolidation tab** (see below) — the two features answer different questions.

**Employee-code matching is unsafe for this specific cross-hotel pension check** — unlike CFEM's own report (where `cfeCodeIndex` is a documented, narrow fallback), CSL's and NL's own auto-generated employee codes genuinely collide with unrelated CFEM codes by coincidence. Confirmed live: CSL's own pension schedule has SAN001/MAK001/THA001/BAA001 too, but they're real CSL employees (Dikeledi Sanyumba, Mary Makina, Olopeng Thapelo, Dennis Baani), not CFE Management — a naive code match would wrongly strip real CSL staff out of CSL's own pension total. `isEmbeddedCfePensionLine()` matches by name only (plus the PHO001 override).

**`matchCfeEmployee()` bug fix**: the first-initial check didn't exclude the matched surname token, so a surname that happens to start with the same letter as the required initial could pass with no real first-name match at all — confirmed live: NL's own "Modimoosi Baakile" (a real NL employee) satisfied the old check purely because "Baakile" itself starts with "B", the same initial as CFEM's "Baboloki Baakile". Fixed by computing the first-initial check only over tokens that aren't part of the matched surname.
...
**Rendering**: rather than storing 5 separate `recon_uploads` rows, the one `cfem_deductions` upload is stored as-is and its sections are converted into `ParsedStatement` shapes **at render time** (`cfemStatements`, keyed by vendor type) whenever `isCfem`. These `cfemStatements` still feed `furnmartStmt`/`afritecStmt`/`toplineStmt`/`cbStmt`/`boduloStmt`, but **the generic "Summary — Statement vs Payroll" table and "Employee Detail" table are hidden entirely for CFEM** (`!isCfem &&` guards on both, plus the unmatched-entries callout and the Management-section block) — CFEM never has a `payroll` upload in this system at all, so a "Payroll" column would only ever show blank/zero, which is exactly the confusing "displaying it as statement and payroll zero" behaviour that prompted this. **The CFE Cross-Reference section (below) is CFEM's only comparison view**, relabelled "Summary — CFEM Report vs CSL/NL Statements" to make clear it's the primary content, not a bolt-on: CFEM's own report acts as the payroll-equivalent source of truth on one side, CSL's/NL's shared vendor statements (filtered to CFE employees) as the "statement" side on the other. Below the vendor-total table, a per-employee detail table (one per vendor, `cfeCrossCheck[].details`) lists **every** CFE employee found on either side, so a genuine 1:1 comparison is visible per person, not just aggregated totals.

**CFE Cross-Reference** (CFEM tab only, below the main Summary table) — the actual "merge the cross reference" feature: a `useEffect` loads CSL's and NL's own `recon_uploads` (the 5 vendor types, **plus `pension`** — see the Pension Contributions section above for why) for the *same year/month* being viewed on the CFEM tab (`csnStmtsForCfe` state, keyed `{ CSL, NL }`). For each vendor, lines from both hotels' statements are filtered down to CFE Management employees via `matchCfeEmployee()` (or, for pension specifically, `isEmbeddedCfePensionLine()`) — the result (`cfeCrossCheck`) is a per-vendor table: CFEM's own report total vs the matched total found embedded in CSL/NL's statements, plus a difference. This is a genuine second, independent comparison from the existing "Management (CFE) section" callout on CSL/NL's own Deductions Check tab (which only extracts from CB Stores/Topline's structural `MGMT` sections, not Furnmart/Afritec/Bodulo) — the two aren't reconciled against each other and can legitimately show different subsets. The standalone amber "not matched on both sides" callout that used to sit below this table (listing `onlyInCfem`/`onlyInEmbedded` employees) was removed per explicit request — the per-employee detail table below already shows every employee on either side, including ones missing from one, so the callout was redundant.
...
- **Pension is deliberately NOT moved between CSL/NL and CFEM here**, even though CFE Management employees are confirmed embedded in NL's own pension schedule (see the Pension Contributions section above) — an earlier version of this tab moved that money into CFEM's figure, which was backwards: whichever hotel's own file a pension line physically sits on is exactly which bank account pays it, not which payroll the employee administratively belongs to, and NL is its own separate account from CSL/CFEM. Each hotel's System pension figure is simply its own uploaded schedule's `bankTotal`, unadjusted. (The CFE Cross-Reference's per-employee Pension visibility on the Deductions Check tab is unaffected — that's an informational comparison, not a bank-account reconciliation, and still surfaces the NL-embedded lines.)

**Figures show cents (2 decimal places), not whole units** — unlike `fmt()`/`fmtCurrency()` everywhere else in the app (which round to whole Rand/Pula by design), this tab uses dedicated `fmtCents()`/`fmtDiffCents()` formatters and rounds `consolidationSystemValue()`/`consolidationBankValue()` to cents rather than whole units. This is deliberate: whole-unit rounding on a bank reconciliation tab could either mask a genuine sub-Rand/Pula gap or manufacture a phantom ±1 "Balance Differential" purely from rounding noise — exactly the kind of thing this tab exists to catch, not hide.
...
**Export to Excel** reuses the existing generic `exportReport()` from `src/lib/reports-export.ts` (the same exporter the Reports page and Leave Provision page use) rather than a bespoke exporter — one sheet, one row per hotel/subtotal per sub-row plus a grand-totals row. `ReportSheet` gained an optional `numberFormat` field (defaults to `'#,##0'`, whole units, everywhere else that reuses this exporter) — Consolidation passes `'#,##0.00'` so the export shows cents too, without changing any other page's export. Filename: `Consolidation_{Month}_{Year}.xlsx`.
...
**A second, simpler Pom Pom export shape exists — an HR-produced sheet, not the full payroll-system export above — and `parsePomPomPayrollXlsx` now handles both.** Confirmed live on `PP_Staff Payroll_August 2026_HR_.xlsx`: just `Last Name / First Name / Emp. Code / Omang / Basic Pay / Other Payments / PAYE / Pension EE`, no `Total Allowances`/`Total Deductions`/`Total Net Pay`/`Leave Pay`/`Overtime`/`Tip Pom Pom` columns at all. Because the header row still starts `"Last Name"`/`"First Name"` it was already being routed into this parser, but the original column detectors (`"Emp. Number"`, `"Pension Employee"`, `"Total Allowances"`) didn't match this file's labels (`"Emp. Code"`, `"Pension EE"`, no allowances column) — silently extracting Basic Salary only and leaving code/pension/PAYE/other-income at 0, with no error. Fixed by: widening the code-column match to `/emp\.?\s*(number|no|code)/` and the pension-column match to a broad `col('pension')` (both original and HR labels contain "pension" and nothing else on the sheet does); adding a direct `"Other Payments"` column match — used as-is for `otherPayments` (with `incomeTotal = basic + otherPayments`) in preference to the Leave/Overtime/Tip derivation when present; making the local `n()` amount parser tolerant of comma-formatted/padded strings (`" 2,940.00 "`) and `null` cells (blank PAYE = no tax that period, not a parse failure); and widening totals-row detection to also fire on Basic/PAYE/Pension being nonzero, since this shape has none of the three columns the original detection relied on. Verified against the real file: all 42 employees parsed, and summed Basic/PAYE/Pension (223,295.97 / 8,651.84 / 11,164.80) match the sheet's own printed totals row exactly.
...
**ILG (and, less commonly, CFEM) can also export a third format: a "LIST OF:" sectioned Payroll List** — the same `"LIST OF: <Vendor> METHOD NO: ALL (Current period)"` plain-text shape Reconciliation's CFEM deductions reports use (`parseCfemDeductions`), but carrying full payroll sections rather than just deductions, and self-contained (no DB enrichment needed, unlike CFEM's RPRT739 above). `isIlgPayrollListFile()`/`parseIlgPayrollList()`/`parseIlgListSections()` in `recon-parsers.ts` handle it; `handleUpload()` checks it after `isRprt739File`/`isIlgAnalysisReportFile` and before falling back to `parsePayrollXlsx`, so it works transparently on **either** ILG's own slot or CFEM's combined-group slot. Confirmed against two real files with different vendor-label spellings for the same concepts:
- ILG's own export: three sections — **"Salary"** (basic — a narrower 2-column shape, just `EMP.CODE`/`EMPLOYEE NAME`/`EMP.AMOUNT`, no `CO.CONTRIB`/`TOTAL` since there's no employer side to a salary line), **"PAYE"**, and **"PROVIDENT"** (both 4-column: `EMP.CODE`/`NAME`/`CO.CONTRIB`/`EMP.AMOUNT`/`TOTAL`).
- CFEM's own export: same shape, different labels — **"Basic Salary"** instead of "Salary", **"Paye"**, **"Pension Fund"** instead of "PROVIDENT" — plus extra **"Severance N/Tax"**, **"Severance Taxed"**, and **"Commission"** sections ILG's doesn't have (all three empty on the confirmed sample, but wired into `PayrollLine.bonusCommission`/`severanceNonTaxable` when present).
- **CFEM's export was revised (2026-09) to add a "Housing Allowance" section** — same narrow 2-column shape as Salary (no `CO.CONTRIB`/`TOTAL`, single `EMP.AMOUNT` value), matched via `findIlgListSection(sections, ['housing'])` and carried on `PayrollLine.housingBenefit` (undefined/0 for every other source — ILG's own export, CSL/NL, Pom Pom, CFEM's older RPRT739 shape — none of which have an equivalent column). Maps to the ITW8 **`BenefitsHousing`** field in `buildItw8Csv()` (previously always hardcoded `'0'`). Shown as its own **Housing Benefit** column on the BURS page's Combined Taxpayers table only (not the ILG tab, since only CFEM's format carries this section) — confirmed against the live file: the section exists but was empty that period (0 employees), so every row correctly parses `housingBenefit: 0` rather than erroring or misreading an adjacent section.
- **Section lookup is substring-based** (`findIlgListSection()`; e.g. `salary` matches both "Salary" and "Basic Salary", `provident`/`pension` covers both spellings), not an exact key match — an earlier version used exact keys and would have silently resolved every section to empty (a blank payroll, no error) for CFEM's differently-labelled sections despite correctly detecting the file format.
- Only the basic/salary section is guaranteed to list every employee (ILG's had 37; its PAYE section only had 8, since PAYE only lists who owes tax that period) — the salary section drives the employee set, and every other section defaults to 0 for anyone missing from it.
- A terminated employee's row can carry a trailing `"TERM DD/MM/YYYY"` note after their amount — harmless, since name/values are sliced up to the first matched number, well before that text.
- Verified against both real files: computed Basic/PAYE/Pension-EE totals match each file's own printed section totals exactly (ILG: 225331.91 / 21022.12 / 10580.00; CFEM: 696960.00 / 120007.29 / 26792.15).
...
**Taxpayers table columns** (2026-09): alongside Salary/Wages and Tax Deducted, the table also shows **Pension Contrib**, **Commission**, and **Other Income** per employee — `line.pensionEe` and a shared `itw8DerivedFields(line)` helper (in the page, above `buildItw8Csv`) for `bonusCommission`/`otherPayments`, so the on-screen figures and the ITW8 export can never disagree about what those two derived fields mean for a given row.
