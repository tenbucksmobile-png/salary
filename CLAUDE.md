...
**Incident: vacant ANO positions counted a resigned employee's leftover salary in the Dashboard total** — `grade_label = 'ANO'` marks a vacant position (no current employee), but its `salary_records` row is whatever the last real occupant was paid before resigning/terminating; the employee row itself stays `status: 'active'` (it still needs to appear as an open position), so nothing excluded its stale salary figure from `SalarySummaryTable`'s money rollup. Confirmed live: **P227,400** spread across IH, ILRB, ILG, CSL, and APA (ILG alone: P22,700 across 4 vacant positions), silently inflating each hotel's Current Gross/CTC totals with money nobody was actually being paid. `computeEmployeeFigures()` now returns zero for every money figure when `grade_label === 'ANO'` while still counting the position toward headcount (useful for vacancy tracking) — the same "vacant position isn't a real employee" treatment the Bonus Provision page already applied is now also applied on the Dashboard. If a hotel's Dashboard total ever looks too high again, check for an ANO-graded employee with a nonzero latest `salary_records.total_earnings` before assuming a calculation bug.
