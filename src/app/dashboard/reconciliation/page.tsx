...
export default function ReconciliationPage() {
  ...
  function matchPayrollLineForIncrease(r: IncreaseRow, payrollLines: PayrollLine[]): { line: PayrollLine | null; ambiguous: boolean } {
    ...
    const sameSurname = payrollLines.filter(l => nameTokens(l.name).includes(surnameToken));
    // Full first-name token first: payroll often carries an extra given name the list
    // omits (confirmed live: list "Matengu, Mary" vs payroll "MISS IWUZE MARY MATENGU"),
    // and an initial-only check can't tell her apart from "MPHO MATENGU".
    const firstTokens = nameTokens(r.firstName);
    if (firstTokens.length) {
      const byFullName = sameSurname.filter(l =>
        nameTokens(l.name).filter(t => t !== surnameToken).some(t => firstTokens.includes(t)));
      if (byFullName.length === 1) return { line: byFullName[0], ambiguous: false };
    }
    const firstInitial = firstTokens[0]?.[0];
    const candidates = sameSurname.filter(l => {
      const rest = nameTokens(l.name).filter(t => t !== surnameToken);
  ...
  function buildMergedIncreaseTable(rows: IncreaseRow[], payrollLines: PayrollLine[]): MergedIncreaseRow[] {
    return permanentThenFtc([...rows]
      ...
      .map(r => {
        ...
        const payrollNewBasic = p ? p.basic : null;
        let flag: MergedIncreaseRow['flag'];
        if (ambiguous) flag = 'ambiguous';
        else if (!p) flag = 'unmatched';
    ...
    try {
      ...
      for (const a of pendingCommitApprovals) {
        if (a.category === 'termination') {
          ...
          const emp = resolve(a);
          const newBasic = a.detail?.listNew;
